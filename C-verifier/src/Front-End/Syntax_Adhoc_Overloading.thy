theory Syntax_Adhoc_Overloading
imports "~~/src/Tools/Adhoc_Overloading"
keywords "adhoc_overloading'" "no_adhoc_overloading'_all" :: thy_decl
begin
...
structure Data_adhoc_cmd = Theory_Data
  (type T = (string * term list) list list
   val empty = []
   val extend = I
   fun merge (l1, l2) = l1 @ map_filter (fn x2 => if List.exists (fn x1 => x1 = x2) l1 then NONE else SOME x2) l2)

fun adhoc_overloading_cmd raw_args lthy =
  let
    fun const_name ctxt =
  ...
  in
    if null raw_args then
      ...
      in
        lthy
        |> Local_Theory.background_theory (Data_adhoc_cmd.map (cons args))
        |> Local_Theory.declaration {syntax = true, pervasive = false}
             (adhoc_overloading_cmd' true args)
      end
  end;
...
val _ =
  Outer_Syntax.local_theory @{command_keyword adhoc_overloading'}
    ...
    (Parse.and_list1 (Parse.const -- Scan.repeat Parse.term) >> adhoc_overloading_cmd);

val _ =
  Outer_Syntax.local_theory @{command_keyword no_adhoc_overloading'_all}
    "delete adhoc overloading for constants / fixed variables"
    (Scan.succeed [] >> adhoc_overloading_cmd);
