...
theorem wpd_eq_intro: 
  "\<lbrakk> \<And> r. (p\<^sub>1 \<turnstile>\<^sub>n Q\<^sub>1) wp\<^sub>D r = (p\<^sub>2 \<turnstile>\<^sub>n Q\<^sub>2) wp\<^sub>D r \<rbrakk> \<Longrightarrow> (p\<^sub>1 \<turnstile>\<^sub>n Q\<^sub>1) = (p\<^sub>2 \<turnstile>\<^sub>n Q\<^sub>2)"
  by (rel_simp robust; metis curry_conv)
...
lemma  rec_total_rule: 
  ...
  apply (rule wf_fixp_uinduct[where fp=\<mu> and Pre=Pre and B=B])    
...
lemma 1:
 ...
 using assms
  by(rel_auto)
find_theorems "_ \<sqsubseteq> _ \<triangleleft> _ \<triangleright> _"
find_theorems "_ \<sqsubseteq> _ ;;_"
  lemma 2: 
  assumes "(b \<and> p\<Rightarrow> q)\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk> \<sqsubseteq> C\<^sub>1\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>" and "(\<not>b \<and> p\<Rightarrow> q)\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk> \<sqsubseteq> C\<^sub>2\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>" 
  shows "(p \<Rightarrow> q)\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk> \<sqsubseteq> (C\<^sub>1 \<triangleleft> b \<triangleright> C\<^sub>2)\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>"
    by (insert assms) rel_auto
      
lemma 3:
  assumes "(\<lceil>p\<rceil>\<^sub>< \<Rightarrow> \<lceil>s\<rceil>\<^sub>>)\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>\<sqsubseteq>(f\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>)" and"(\<lceil>s\<rceil>\<^sub>< \<Rightarrow>  \<lceil>q\<rceil>\<^sub>>)\<sqsubseteq>(fa)"
...
lemma 4:
  assumes "(\<lceil>p \<and> q\<rceil>\<^sub>< \<Rightarrow> \<lceil>p \<and> q\<rceil>\<^sub>>) \<sqsubseteq> (f)" and"(\<lceil>p \<and> q\<rceil>\<^sub>< \<Rightarrow>  \<lceil>q\<rceil>\<^sub>>) \<sqsubseteq> fa"
  shows "(\<lceil>p \<and> q\<rceil>\<^sub>< \<Rightarrow> \<lceil>q\<rceil>\<^sub>>) \<sqsubseteq> (f ;; fa)"
  using assms 
  by (auto simp: 3)

lemma 5:
  assumes "(\<lceil>p\<rceil>\<^sub>< \<Rightarrow> \<lceil>q\<rceil>\<^sub>>)\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>\<sqsubseteq>f\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>" and"(\<lceil>q\<rceil>\<^sub>< \<Rightarrow>  \<lceil>q\<rceil>\<^sub>>)\<sqsubseteq>fa"
  shows "(\<lceil>p\<rceil>\<^sub>< \<Rightarrow> \<lceil>q\<rceil>\<^sub>>)\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk> \<sqsubseteq> (f ;; fa)\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>"
  using assms 
  by (auto simp: 3)

lemma 6:
  assumes "(\<lceil>p\<rceil>\<^sub>< \<Rightarrow> \<lceil>p\<rceil>\<^sub>>)\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>\<sqsubseteq>(f\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>)" and"(\<lceil>p\<rceil>\<^sub>< \<Rightarrow>  \<lceil>q\<rceil>\<^sub>>)\<sqsubseteq>fa"
  shows "(\<lceil>p\<rceil>\<^sub>< \<Rightarrow> \<lceil>q\<rceil>\<^sub>>)\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk> \<sqsubseteq> (f ;; fa)\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>"
  using assms 
  by (auto simp: 3) 

lemma 7:"`P\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>` \<Longrightarrow> P\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk> \<sqsubseteq> II\<lbrakk>\<guillemotleft>st\<guillemotright>/$\<Sigma>\<rbrakk>"
  by pred_simp
