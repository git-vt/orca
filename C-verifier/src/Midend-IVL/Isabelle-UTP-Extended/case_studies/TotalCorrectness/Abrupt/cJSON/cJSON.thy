...
locale cJSON =
  fixes int_ret :: \<open>int \<Longrightarrow> '\<alpha>\<close>
    and tempstr\<^sub>1 tempstr\<^sub>2 :: \<open>string \<Longrightarrow> '\<alpha>\<close> \<comment> \<open>Assuming already shown non-@{const None}\<close>
  assumes \<open>vwb_lens int_ret\<close>
          \<open>lens_indep_all [tempstr\<^sub>1, tempstr\<^sub>2]\<close>
begin

(* TODO: Block or frame? *)
definition case_insensitive_strcmp :: \<open>'\<alpha> str_lens \<Rightarrow> '\<alpha> str_lens \<Rightarrow> '\<alpha> hrel_cpa\<close> where
...
abbreviation can_read ::
  \<open>('\<alpha> parse_buffer option, '\<alpha>) uexpr \<Rightarrow> (size_t, '\<alpha>) uexpr \<Rightarrow> '\<alpha> upred\<close> where
...
abbreviation can_access_at_index ::
  \<open>('\<alpha> parse_buffer option, '\<alpha>) uexpr \<Rightarrow> (size_t, '\<alpha>) uexpr \<Rightarrow> '\<alpha> upred\<close> where
