...
lemma hoare_false [hoare_total]: "\<lbrace>false\<rbrace>C\<lbrace>q\<rbrace>\<^sub>D"
  by rel_auto

subsection {*Precondition strengthening*}

lemma hoare_pre_str[hoare_total]:
...
lemma hoare_post_weak[hoare_total]:
...
lemma hoare_r_conj [hoare_total]: 
...
lemma skip_hoare_r [hoare_total]: "\<lbrace>p\<rbrace>SKIP\<lbrace>p\<rbrace>\<^sub>D"
  by rel_auto

subsection {*Hoare for assignment*}

lemma assigns_hoare_r [hoare_total]: 
  assumes"`p \<Rightarrow> \<sigma> \<dagger> q`" 
  shows  "\<lbrace>p\<rbrace>\<langle>\<sigma>\<rangle>\<^sub>C\<lbrace>q\<rbrace>\<^sub>D"
  by (insert assms) rel_auto

lemma assigns_hoare_r' [hoare_total]: "\<lbrace>\<sigma> \<dagger> p\<rbrace>\<langle>\<sigma>\<rangle>\<^sub>C\<lbrace>p\<rbrace>\<^sub>D"
  by rel_auto

subsection {*Hoare for Sequential Composition*}

lemma seq_hoare_r [hoare_total]: 
...
lemma cond_hoare_r [hoare_total]: 
...
lemma assert_hoare_r [hoare_total]: 
...
lemma assume_hoare_r [hoare_total]: 
...
lemma while_hoare_r [hoare_total]:
...
lemma while_hoare_r' [hoare_total]:
  ...
  using assms
  by (metis utp_hoare_total.hoare_post_weak utp_hoare_total.while_hoare_r utp_pred.inf_commute)

lemma while_invr_hoare_r [hoare_total]:
  ...
  by (metis assms utp_hoare_total.hoare_pre_str utp_hoare_total.while_hoare_r' utp_fault_designs.while_inv_def utp_pred.inf_commute)
