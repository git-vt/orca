...
lemma id_subst_ext [usubst,alpha]: (*legacy do not need vwb_lens to discharge the proof*)
  "wb_lens x \<Longrightarrow> id \<oplus>\<^sub>s x = id"
  by pred_auto

lemma upd_subst_ext [alpha]: 
  "mwb_lens x \<Longrightarrow> \<sigma>(y \<mapsto>\<^sub>s v) \<oplus>\<^sub>s x = (\<sigma> \<oplus>\<^sub>s x)(&x:y \<mapsto>\<^sub>s v \<oplus>\<^sub>p x)" (*legacy do not need vwb_lens to discharge the proof*)
  by pred_auto

lemma apply_subst_ext [alpha]: 
  "vwb_lens x \<Longrightarrow> (\<sigma> \<dagger> e) \<oplus>\<^sub>p x = (\<sigma> \<oplus>\<^sub>s x) \<dagger> (e \<oplus>\<^sub>p x)" (*legacy do not need vwb_lens to discharge the proof*)
  by (pred_auto)
...
lemma upd_subst_res [alpha]: (*legacy do not need vwb_lens to discharge the proof*)
...
lemma subst_ext_res [alpha,usubst]: (*legacy do not need vwb_lens to discharge the proof*)
