...
lemma unrest_cond [unrest]:
  "\<lbrakk> x \<sharp> P; x \<sharp> b; x \<sharp> Q \<rbrakk> \<Longrightarrow> x \<sharp> IF b THEN P ELSE Q"
  unfolding cond_def
...
lemma usubst_condr [usubst]:
  "\<sigma> \<dagger> (IF b THEN P ELSE Q) = (IF \<sigma> \<dagger> b THEN \<sigma> \<dagger> P ELSE \<sigma> \<dagger> Q)"
  unfolding cond_def
  by transfer' (rel_auto)
 
(*lemma subst_skip_r [usubst]:
  "out\<alpha> \<sharp> \<sigma> \<Longrightarrow> \<sigma> \<dagger> SKIP = \<langle>\<lfloor>\<sigma>\<rfloor>\<^sub>s\<rangle>\<^sub>a"
...
lemma cond_mono:
  "\<lbrakk> P\<^sub>1 \<sqsubseteq> P\<^sub>2; Q\<^sub>1 \<sqsubseteq> Q\<^sub>2 \<rbrakk> \<Longrightarrow> (IF b THEN P\<^sub>1 ELSE Q\<^sub>1) \<sqsubseteq> (IF b THEN P\<^sub>2 ELSE Q\<^sub>2)"
  unfolding cond_def
  by (rel_auto)

lemma cond_monotonic:
  "\<lbrakk> mono P; mono Q \<rbrakk> \<Longrightarrow> mono (\<lambda> X. IF b THEN P X ELSE Q X)"
  unfolding cond_def
  by (simp add: mono_def, rel_blast)
